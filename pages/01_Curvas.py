...
def render_rigid_math_2d(meta, alpha_latex: str, tmin: float, tmax: float):
    ...
    st.latex(
        ...
        rf"{fmt(M[0,0])}x(t)+{fmt(M[0,1])}y(t)+{fmt(a[0])},\;"
...
def render_rigid_math_3d(meta, alpha_latex: str, tmin: float, tmax: float):
    ...
    st.latex(
        rf"\widetilde\alpha(t)=\left("
        rf"{fmt(M[0,0])}x(t)+{fmt(M[0,1])}y(t)+{fmt(M[0,2])}z(t)+{fmt(a[0])},\;"
        rf"{fmt(M[1,0])}x(t)+{fmt(M[1,1])}y(t)+{fmt(M[1,2])}z(t)+{fmt(a[1])},\;"
...
def render_planar_frenet_at_point(
    ...
    """Exibe as equações de Frenet planas avaliadas no ponto escolhido."""
    ...
    if not np.isfinite(kappa):
        st.warning("A curvatura não está definida numericamente neste ponto.")
        return
    ...
    st.latex(
        rf"\alpha(s_0)={vector_latex(p)},\qquad "
        rf"\mathfrak{{t}}(s_0)={vector_latex(T)},\qquad "
        rf"\mathfrak{{n}}(s_0)={vector_latex(N)}"
    )
    st.latex(rf"\kappa(s_0)={latex_scalar(kappa)}")
    st.latex(
        rf"\alpha'(s_0)=\mathfrak{{t}}(s_0)={vector_latex(alpha_s)}"
    )
    st.latex(
        rf"\mathfrak{{t}}'(s_0)=\kappa(s_0)\mathfrak{{n}}(s_0)"
        rf"={vector_latex(tangent_s)}"
    )
    st.latex(
        rf"\mathfrak{{n}}'(s_0)=-\kappa(s_0)\mathfrak{{t}}(s_0)"
        rf"={vector_latex(normal_s)}"
    )


def render_spatial_frenet_at_point(
    p: np.ndarray,
    T: Optional[np.ndarray],
    N: Optional[np.ndarray],
    ...
    if T is None:
        ...
        return
    if N is None or B is None or not np.isfinite(kappa):
        st.warning(
            "O triedro de Frenet não está definido neste ponto, pois "
            r"$\alpha'(t_0)\times\alpha''(t_0)=0$ ou a curvatura não está definida."
        )
        return
    ...
    st.latex(
        rf"\alpha(s_0)={vector_latex(p)},\qquad "
        rf"\mathfrak{{t}}(s_0)={vector_latex(T)}"
    )
    st.latex(
        rf"\mathfrak{{n}}(s_0)={vector_latex(N)},\qquad "
        rf"\mathfrak{{b}}(s_0)={vector_latex(B)}"
    )
    st.latex(
        rf"\kappa(s_0)={latex_scalar(kappa)},\qquad "
        rf"\tau(s_0)={latex_scalar(tau)}"
    )
    st.latex(
        rf"\alpha'(s_0)=\mathfrak{{t}}(s_0)={vector_latex(alpha_s)}"
    )
    ...
    if np.isfinite(tau):
        normal_s = -kappa * T + tau * B
        binormal_s = -tau * N
        st.latex(
            rf"\mathfrak{{n}}'(s_0)"
            rf"=-\kappa(s_0)\mathfrak{{t}}(s_0)"
            rf"+\tau(s_0)\mathfrak{{b}}(s_0)"
            rf"={vector_latex(normal_s)}"
        )
        st.latex(
            rf"\mathfrak{{b}}'(s_0)"
            rf"=-\tau(s_0)\mathfrak{{n}}(s_0)"
            rf"={vector_latex(binormal_s)}"
        )
    else:
        st.warning(
            "A torção não está definida neste ponto; por isso, não calculamos "
            r"$\mathfrak{n}'(s_0)$ e $\mathfrak{b}'(s_0)$."
        )
...
def render_planar_analysis():
    ...
    render_planar_frenet_at_point(p, T, N, kap)
...
def render_spatial_analysis():
    ...
    render_spatial_frenet_at_point(p, T, N, B, kap, tau)
...
def render_planar_theorem():
    ...
    with st.expander("Cálculos do sistema de Frenet no ponto escolhido", True):
        render_planar_frenet_at_point(alpha[i], T[i], N[i], k[i])
...
st.markdown(
    "Módulo interativo baseado no capítulo de curvas: exemplos, regularidade, "
    "comprimento de arco, referenciais de Frenet, movimentos rígidos e "
